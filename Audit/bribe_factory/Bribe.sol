...
contract Bribe is ReentrancyGuard, Ownable {
    ...
    function _earned(address _voter, address _rewardToken, uint256 _timestamp) public view returns (uint256) {
   ...
   //---------------------------
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external nonReentrant {
        ...
        rewardData[_rewardsToken][_startTimestamp].periodFinish = getEpoch() + 1;
    ...
    function getRewardForOwnerToOtherOwner(address _voter, address _receiver) public nonReentrant  {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            if (reward > 0) {
                ...
                address _mainRefFeeReceiver = IGaugeFactory(gaugeFactory).mainRefFeeReceiver();
                IERC20(_rewardToken).safeTransfer(_mainRefFeeReceiver,remainingRefReward);
                earnedRefs[_mainRefFeeReceiver][_rewardToken] = earnedRefs[_mainRefFeeReceiver][_rewardToken] + remainingRefReward;
                emit RefRewardPaid(_mainRefFeeReceiver, _rewardToken, remainingRefReward);
                }
            }
